  ...
  return (
    <div className="fixed inset-0 z-50 bg-black">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-card rounded-xl shadow-xl p-4 max-w-lg z-10">
        ...
        {autoDetectMessage && (
          <div className="mt-3 p-2 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-lg flex items-center gap-2">
            <svg className="w-4 h-4 text-amber-500 dark:text-amber-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <span className="text-amber-800 dark:text-amber-300 text-sm">{autoDetectMessage}</span>
            <button
              onClick={() => setAutoDetectMessage(null)}
              className="ml-auto p-1 hover:bg-amber-100 dark:hover:bg-amber-900/50 rounded"
            >
